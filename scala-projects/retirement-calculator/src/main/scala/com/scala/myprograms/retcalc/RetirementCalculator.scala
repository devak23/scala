...
object RetirementCalculator {
  ...
  def futureCapital(rateOfInterest: Double
    ...
    /* We iterate through the months and apply foldLeft which is similar to a reduce function used in most of the
     * programming languages. The foldLeft has two parameter lists. Scala allows you to have many parameter lists.
     * Each list can have one or many parameters. This does not change the behavior of the function rather, its just a
     * way of separating the concerns. The foldLeft takes an operator function (next capital) and acts on the accumulator
